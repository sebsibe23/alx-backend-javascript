...
/**
 * A simple HTTP server that provides student data from a CSV file.
 *
 * This script creates an HTTP server using the Node.js `http`
 * module. The server listens on a specified port and host, and
 * responds to incoming requests with either a greeting message
 * or a list of students from a CSV file.
 *
 * The server is configured to listen on port 1245 and the host
 * 'localhost'. When a request is received, the server routes
 * the request to the appropriate handler based on the URL path.
 *
 * Routes:
 * - `/`: Responds with "Hello Holberton School!".
 * - `/students`: Reads a CSV file and responds with the list of
